#!/usr/bin/env python3
"""
Eviscerating Angels V4 – Dark Narrative
GCE 9.5+ target. Transform into fully realised dark narrative.

Core motif: A3-E4-G4 (from opening). Apply inversion, displacement, expansion, fragmentation.
Four phases: A ominous, B unstable, C violent climax, D exhausted resolution.
String counterpoint: Vln1 dramatic, Vln2 echo/tension, Viola contrary, Cello gravity.
Guitar: colour instrument — dark dyads, low punctuations.
"""
import os
from music21 import (
...
# Phase map: A(1-18) B(19-36) C(37-54) D(55-72)
def phase(m):
    if m <= 18: return "A"
    if m <= 36: return "B"
    if m <= 54: return "C"
...
# === CORE MOTIF: A3-E4-G4. Inversion: G4-E4-A3. Frag: A3-E4, E4-G4. Expansion: +B4 ===

# === GUITAR V4: Colour instrument — dark dyads, low punctuations ===
def guitar_content(sec, bar, m):
    ph = phase(m)
    bar_ph = bar_in_phase(m)
    if ph == "A":
        if bar_ph in (0, 2, 5, 8, 11):
            return [r(5)], None
    ...
    if ph == "C":
        ...
        return [c(pit, 2.5), r(2.5)], chord_symbol_for(pit)
    if ph == "D":
        if bar_ph in (0, 2, 4, 6, 8, 10, 12, 14, 16):
            return [r(5)], None
        blocks = [("A3", "E4"), ("G3", "D4"), ("E4", "B4"), ("A3", "E4")]
        pit = blocks[bar_ph % 4]
        return [c(pit, 3), r(2)], chord_symbol_for(pit)
    return [r(5)], None

# === VIOLIN I: Primary dramatic line — motif transformations ===
def violin1_content(sec, bar, m):
    ...
    if ph == "C":
        ...
        if bar < 16:
            return [n("B5", 1), n("A5", 1), n("G5", 1.5), r(1.5)]
...
# === VIOLIN II: Echo fragments, rising tension ===
...
# === VIOLA: Contrary motion, harmonic glue ===
...
# === CELLO: Slow gravity, pedal anchors ===
def cello_content(sec, bar, m):
    ph = phase(m)
    bar_ph = bar_in_phase(m)
    if ph == "A":
        if bar_ph in (0, 1, 2, 4, 5, 7, 8, 10):
            return [n("E2", 2), r(3)]
...
def build_score():
    ...
    for m in range(1, 73):
        ...
        dyn_map = {1: "pp", 19: "p", 25: "mp", 37: "mf", 43: "f", 49: "ff", 55: "mf", 61: "p", 67: "pp"}
        if m in dyn_map:
...
def main():
    ...
    out_ecm = os.path.join(ws_root, "ECM-Orbit-Album-2027", "Compositions", "Eviscerating_Angels", "V4_Eviscerating_Angels_DarkNarrative.musicxml")
    os.makedirs(os.path.dirname(out_ecm), exist_ok=True)

    out_local = os.path.join(base, "musicxml", "V4_Eviscerating_Angels_DarkNarrative.musicxml")
