...
"""
Long Echo V1
Tonality Vault / Frisell Atmosphere Hybrid
Target: GCE ≥ 9.8

Album epilogue. Extremely sparse, spacious, harmonically suspended.
Form: A (Distant emergence) B (Slow bloom) C (Glass suspension) D (Thinning) E (Final echo)
Duration: ~5 min at ♩ = 52
"""
import os
...
# === GUITAR: 35-45% dyads, 25-35% triads, 15-20% sustained, 10-15% single ===
def guitar_a(bar):
    if bar in (0, 1, 4, 5):
...
def viola_content(sec, bar, m):
    ...
    if sec == "C":
        if bar % 2 == 0:
            return [n("A3", 1), n("B3", 1), n("C#4", 1), r(1)]
        return [r(4)]
    if sec == "D":
        if bar < 7:
            return [n("D4", 3), r(1)]
...
def use_reduced_texture(m, sec):
    if sec == "A":
        return m in (1, 2, 3, 6, 7, 10, 11)
    if sec == "B":
        return m in (14, 15, 20, 21, 24, 25)
    if sec == "C":
        return m in (28, 29, 34, 35, 40, 41)
    if sec == "D":
        return m >= 50
    if sec == "E":
        return True
...
def build_score():
    ...
    for m in range(1, 67):
        ...
        if reduced and not is_pre_ending(m):
            if m % 3 == 0:
                vn2_c = [r(4)]
            if m % 3 == 1 and sec != "A":
                vla_c = [r(4)]
...
def main():
    ...
    out_path = os.path.join(out_dir, "V1_Long_Echo.musicxml")
