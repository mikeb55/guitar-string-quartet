#!/usr/bin/env python3
"""
Labyrinth of Quiet Motions V2 Final
Refinement of V1 — GCE 9.8+

Refinements:
- Section C extended by 5 measures; guitar in counterpoint; more contrary motion
- Motivic seed reintroduced in Violin I at midpoint; guitar-viola echo
- Section D extended by 3 measures for gradual dissolution
- Ending: 1 measure space before final sonority
"""
import os
...
# === FORM (refined: C +5, D +3, E +1 space) ===
def section(m):
    if m <= 18: return "A"
    if m <= 40: return "B"
    if m <= 83: return "C"   # was 78, now 83 (+5)
    if m <= 103: return "D"  # was 98, now 103 (+3)
    return "E"               # 104-119
...
def guitar_c(bar):
    """C: Guitar participates in counterpoint — continuous dyadic/triadic motion"""
    # Midpoint bars 59-60: echo motive E-G-A-D with viola
    if bar in (18, 19):
        return [
            [("E3", "G3"), ("A3", "D4"), ("G3", "A3"), ("E3", "G3")],
...
def guitar_d(bar):
    """D: Gradual dissolution — more gradual thinning"""
    if bar < 6:
        frags = [
            [("E3", "G3"), ("A3",), ("G3", "A3"), ("rest",)],
...
# === VIOLIN I: Motivic seed at midpoint (bar 59-60 = C bars 18-19) ===
...
# === VIOLIN II: More contrary motion in C ===
...
# === VIOLA: Echo motive at midpoint; more contrary motion ===
...
def build_score():
    ...
    section_starts = [1, 19, 41, 84, 104]
    section_labels = ["A", "B", "C", "D", "E"]
    section_double_barlines = [18, 40, 83, 103]
...
def main():
    ...
    out_path = os.path.join(out_dir, "V2_Labyrinth_of_Quiet_Motions_Final.musicxml")
