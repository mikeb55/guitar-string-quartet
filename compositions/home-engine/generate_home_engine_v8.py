#!/usr/bin/env python3
"""
Home Engine V8 - Guitar and String Quartet
Target: GCE ≥ 9.8–10. Scofield–Holland + Zappa (minimal).

V8 micro-refinements over V7:
- MOTIVIC INEVITABILITY: D-F-A interval pattern in 4+ contexts; fragments pass
  between guitar, violin I, cello. Rhythmic mutation, no new unrelated material.
- INNER-VOICE REFINEMENT: Viola/Vn2 stepwise voice-leading; smooth chord-tone
  connections during harmonic pivots; avoid large leaps.
- CLIMAX SHAPE: Vn1 to G6 (peak); wider vertical spacing; guitar stacked dyads/triads;
  cello anchors arrival. Post-climax: density drop, suspended calm, motif return.
- RHYTHMIC FLEXIBILITY: Anticipatory hits, displaced accents, 5/7-bar asymmetry.
- ANTI-MONOTONY: Cycle 6 bars; register exchange, dyad inversion, handoff.
...
def guitar_e(bar):
    """E: Climax — stacked dyads/triads, widest spacing; bars 77-80 release"""
    if bar >= 14:
        if bar % 2 == 1:
            return "rest"
        return ("D3", "F3", "A3")
...
# === VIOLA: Stepwise voice-leading, smooth harmonic pivots ===
...
# === VIOLIN I: Motif fragments, climax G6 (peak), handoff to cello ===
...
# === CELLO: Harmonic gravity, motif fragments (D-A-F), anchors climax ===
...
def main():
    score = build_score()
    out = os.path.join(os.path.dirname(__file__), "musicxml", "V8_Home_Engine_Scofield.musicxml")
