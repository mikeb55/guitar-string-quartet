#!/usr/bin/env python3
"""
Home Engine V5 - Guitar and String Quartet
Target: GCE ≥ 9.8. Scofield–Holland + Zappa (brief).

V5 improvements over V4:
- Harmonic clarity: chord symbols every 2-4 bars
- Guitar–violin dialogue: answer/echo phrasing, motif fragments pass between
- Climax reinforcement: widest spacing, all instruments active, Vn1 to E6
- Cello: stepwise motion, anticipatory notes, pedal→resolution
- Anti-monotony: variation every 4-6 bars, register shift, ensemble dropouts
...
# Harmonic clarity — chord changes every 2-4 bars; more density
...
# Chord symbol every 2 bars for clarity (force insert when bar % 2 == 0)
...
# Rhythmic vitality — 10 patterns; variation every 4 bars
RHYTHM = [
    [0.5, 1.0, 0.5, 2.0],
    [1.0, 0.5, 1.5, 1.0],
    [0.5, 1.5, 0.5, 1.5],
...
def get_rhythm(m, sec, bar):
    idx = ((m - 1) // 4 + bar + hash(sec) % 10) % len(RHYTHM)
    return RHYTHM[idx]

# Guitar — dyads/triads; dialogue: answer bars (after vn), echo bars
def guitar_a(bar):
    blocks = [
...
def guitar_e(bar):
    """E: Climax — stacked triads, quartal, widest register"""
    blocks = [
...
# Viola — continuous interior counterline; dropout every 6 bars for variation
...
# Violin I — dialogue with guitar; climax: E6 (highest register)
def violin1_content(sec, bar, m):
    ...
    if sec == "E":
        ...
        if bar < 8:
            return [n("E6", 1), r(1), c(("Bb5", "D6"), 1), r(1)]
        if bar < 12:
...
# Violin II — active participation; secondary melodic; echo guitar
...
# Cello — Dave Holland: stepwise motion, anticipatory notes, pedal→resolution
...
def main():
    score = build_score()
    out = os.path.join(os.path.dirname(__file__), "musicxml", "V5_Home_Engine_Scofield.musicxml")
