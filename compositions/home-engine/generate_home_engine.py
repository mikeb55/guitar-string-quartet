#!/usr/bin/env python3
"""
Home Engine - Guitar and String Quartet
Engine: Scofield–Holland Groove. Secondary: Zappa Disruption (limited).
~5 min, 108 bars @ 4/4, q=104.

Scofield–Holland: groove-based harmony, syncopated guitar, strong bass, ensemble dialogue
Zappa: Section D only — ensemble hits, rhythmic fractures, unexpected harmonic stack

Form: A (1-20) B (21-40) C (41-60) D (61-76 Zappa) A' (77-96) Coda (97-108)
Harmony: Dm9, G13sus, Bbmaj7#11, Fmaj9, A7alt
"""
import os
from music21 import (
...
# === SECTION MAP (108 bars, ~5 min) ===
# A: 1-20, B: 21-40, C: 41-60, D: 61-76 (Zappa), A': 77-96, Coda: 97-108
def section(m):
    if m <= 20: return "A"
    if m <= 40: return "B"
    if m <= 60: return "C"
...
# Syncopated rhythm patterns (Scofield) — 4/4
RHYTHM = [
    [1.0, 0.5, 1.0, 1.5],
    [0.5, 1.5, 0.5, 1.5],
    [1.5, 0.5, 1.0, 1.0],
...
def guitar_c(bar):
    """C: Contrapuntal expansion — Bbmaj7#11 / Fmaj9"""
    blocks = [
...
def guitar_d(bar):
    """D: Zappa — ensemble hits, rhythmic fractures, unexpected harmonic stack"""
    # Zappa: staccato chord punctuations, angular voicings, fractured rhythm
    blocks = [
...
def guitar_a2(bar):
    """A': Transformed return — groove restored"""
    blocks = [
...
def viola_content(sec, bar, m):
    ...
    if sec == "C":
        ...
        return lines[bar % 3]
    if sec == "D":
        # Zappa: ensemble hits — tutti staccato, fractured
        return [
...
def violin1_content(sec, bar, m):
    ...
    if sec == "C":
        ...
        return [r(4)]
    if sec == "D":
        # Zappa: ensemble hit — angular fragment
...
def violin2_content(sec, bar, m):
    ...
    if sec == "D":
        # Zappa: rhythmic echo — off-beat hit
...
def cello_content(sec, bar, m):
    ...
    if sec == "D":
        # Zappa: fractured bass — unexpected accents
...
def build_score():
    ...
    for m in range(1, 109):
        ...
        if m == 61:
            mg.insert(0, dynamics.Dynamic("ff"))  # Zappa ensemble hits
        elif m in (21, 41):
            mg.insert(0, dynamics.Dynamic("mp"))
        elif m == 77:
            mg.insert(0, dynamics.Dynamic("mp"))  # Return to groove
        elif m == 97:
...
def main():
    score = build_score()
    out = os.path.join(os.path.dirname(__file__), "musicxml", "Home_Engine_Scofield_Holland.musicxml")
