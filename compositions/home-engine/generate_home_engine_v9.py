#!/usr/bin/env python3
"""
Home Engine V9 - Guitar and String Quartet
Target: GCE ≈ 9.8–10. Scofield–Holland + Zappa (very minimal).

V9 refinements over V8:
- MOTIVIC DNA: D-F-A embedded in later phrases; cello/viola subtle fragments;
  rhythmic mutation only; no unrelated ideas late.
- VOICE-LEADING ELEGANCE: Viola/Vn2 stepwise; smooth chord-tone connections;
  reduce leaps during harmonic transitions.
- CLIMAX INEVITABILITY: Before apex—gradual tension, wider spacing, rhythmic
  intensity. At peak—Vn1 G6, guitar stacked, cello anchors. After—density drop,
  suspended calm, motif return.
- RHYTHMIC REFINEMENT: 5/7-bar phrase asymmetry; displaced accents.
- ANTI-MONOTONY: Cycle 6 bars; handoff, register exchange, dyad inversion.
...
# Rhythmic flexibility: anticipatory, displaced, 5/7-bar asymmetry
RHYTHM = [
    [0.5, 1.0, 0.5, 2.0],
    [1.5, 0.25, 0.25, 2.0],
    [0.25, 1.5, 0.5, 1.75],
...
# === MOTIF D-F-A: 4+ transforms; pass guitar → Vn1 → cello ===
def guitar_a(bar):
...
def guitar_e(bar):
    """E: Climax — bars 63-64 build, 65-68 peak, 77-80 release"""
    if bar >= 14:
        if bar % 2 == 1:
            return "rest"
        return ("D3", "F3", "A3")
...
# === VIOLA: Stepwise voice-leading, motif fragments (D-F-A) ===
def viola_content(sec, bar, m):
    ...
    if sec == "C":
        if bar % 7 == 4:
            return [n("D4", 0.5), r(0.5), n("F4", 0.5), r(0.5), n("A4", 1), r(1.5)]
        lines = [
    ...
    if sec == "A2":
        if bar % 5 == 3:
            return [n("D4", 1), r(0.5), n("F4", 1), r(0.5), n("A4", 1), r(1)]
        return [c(("D3", "F3", "A3"), 1.5), r(0.5), n("C4", 1), r(1)]
    if sec == "Coda":
        return [n("D4", 2), r(2)] if bar < 10 else [r(4)]
    return [r(4)]

# === VIOLIN I: Motif fragments, climax G6 (peak) ===
def violin1_content(sec, bar, m):
    ...
    if sec == "E":
        ...
        if bar < 6:
            return [n("G6", 1), r(1), c(("Bb5", "D6"), 1), r(1)]
        if bar < 8:
...
# === VIOLIN II: Stepwise, dialogue, smooth chord-tone connections ===
...
# === CELLO: Harmonic gravity, motif fragments (D-F-A), anchors climax ===
def cello_content(sec, bar, m):
    ...
    if sec == "B":
        if bar % 5 == 2:
            return [n("D2", 1), r(0.5), n("F2", 0.5), r(0.5), n("A2", 1), r(1)]
        patterns = [
...
def main():
    score = build_score()
    out = os.path.join(os.path.dirname(__file__), "musicxml", "V9_Home_Engine_Scofield.musicxml")
