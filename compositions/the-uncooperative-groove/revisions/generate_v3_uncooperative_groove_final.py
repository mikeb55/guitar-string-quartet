#!/usr/bin/env python3
"""
The Uncooperative Groove V3 – Micro-upgrade from V2 (GCE 9.8+)
Orbit Album – Guitar + String Quartet

Surgical refinements:
1. Groove fingerprint: cello E-A-E pattern recurs in A, C, D (transformed)
2. Climax extended 6 bars (E: 47-70), increased density, ensemble hits
3. Signature gesture: bar 71 full-stop unison hit, bar 72 silence, then F

Form: A(1-8) B(9-16) C(17-30) D(31-46) E(47-70) [SIG 71] [SILENCE 72] F(73-76)
~76 bars. ♩=104.
"""
import os
from music21 import (
...
def time_sig(m):
    sec = section(m)
    if sec in ("SIG", "SILENCE"): return "4/4"
    if sec == "A": return "7/8"
    if sec == "B": return "5/8"
    if sec == "C": return "4/4"
    if sec == "D":
...
def bar_dur(m):
    sec = section(m)
    if sec == "SIG": return 4.0
    if sec == "SILENCE": return 4.0
    ts = time_sig(m)
    if ts == "7/8": return 3.5
    if ts == "5/8": return 2.5
    return 4.0

def chord_for(sec, bar, m):
    if sec in ("SIG", "SILENCE", "F"): return None
...
# === GROOVE FINGERPRINT: E-A-E cell (cello) — long-short-long. Recur in A, C, D ===
def guitar_content(sec, bar, m):
    bd = bar_dur(m)
    if sec == "SIG": return [c(("E3", "Bb3", "G3"), 0.25), r(3.75)]
    if sec == "SILENCE": return [r(4)]
    if sec == "A":
    ...
    if sec == "E":
        if bar in (0, 2, 5, 7, 10, 12, 15, 17): return [r(bd)]
        blocks = [
...
def violin1_content(sec, bar, m):
    bd = bar_dur(m)
    if sec == "SIG": return [n("G5", 0.25), r(3.75)]
    if sec == "SILENCE": return [r(4)]
    if sec == "A":
        blocks = [
    ...
    if sec == "D":
        ...
        return [r(bd)]
    if sec == "E":
        if bar in (1, 4, 8, 11, 15, 19, 21): return [n("G5", 0.5), n("Bb5", 0.5), r(bd - 1)]
        if bar in (3, 6, 9, 13, 17, 20, 23): return [n("E5", 0.5), r(0.5), n("G5", 1), r(bd - 2)]
        if bar in (14, 16, 18, 22): return [n("G5", 0.25), n("Bb5", 0.25), n("E5", 0.25), r(bd - 0.75)]
        return [r(bd)]
    if sec == "F":
        if bar == 2:
            nh = n("B5", 2)
...
def violin2_content(sec, bar, m):
    bd = bar_dur(m)
    if sec == "SIG": return [n("E5", 0.25), r(3.75)]
    if sec == "SILENCE": return [r(4)]
    if sec == "A":
    ...
    if sec == "D":
        ...
        return [r(bd)]
    if sec == "E":
        if bar in (3, 7, 11, 15, 19, 21): return [r(0.5), n("G4", 1), r(bd - 1.5)]
        if bar in (14, 16, 18, 22): return [n("G4", 0.25), n("Bb4", 0.25), r(bd - 0.5)]
        return [r(bd)]
    return [r(bd)]

def viola_content(sec, bar, m):
    bd = bar_dur(m)
    if sec == "SIG": return [n("C#4", 0.25), r(3.75)]
    if sec == "SILENCE": return [r(4)]
    if sec == "A":
    ...
    if sec == "D":
        ...
        return [r(bd)]
    if sec == "E":
        if bar in (2, 6, 10, 14, 18, 20, 22): return [n("C#4", 1.5), r(bd - 1.5)]
        if bar in (14, 16, 18, 22): return [n("C#4", 0.25), n("E4", 0.25), n("G4", 0.25), r(bd - 0.75)]
        return [r(bd)]
    return [r(bd)]

def cello_content(sec, bar, m):
    bd = bar_dur(m)
    if sec == "SIG": return [n("E2", 0.25), r(3.75)]
    if sec == "SILENCE": return [r(4)]
    if sec == "A":
        pats = [
    ...
    if sec == "E":
        if bd >= 3:
            if bar in (14, 16, 18, 20, 22):
                return [n("E2", 0.5), n("Bb2", 0.5), n("E2", 0.5), n("A2", 0.5), r(bd - 2)]
            return [n("E2", 1), n("Bb2", 1), n("E2", 1), r(bd - 3)]
...
def build_score():
    ...
    section_starts = [1, 9, 17, 31, 47, 71, 73]
    section_labels = ["A", "B", "C", "D", "E", "", "F"]
    dyn_map = {1: "mp", 9: "mf", 17: "mp", 31: "mf", 47: "f", 65: "ff", 71: "ff", 73: "p"}
    ...
    for m in range(1, 77):
        ...
        if m in section_starts:
            ...
            if section_labels[idx]:
                mg.insert(0, expressions.RehearsalMark(section_labels[idx]))
        if m == 71:
            mg.insert(0, expressions.TextExpression("full stop"))
        if m in dyn_map:
            mg.insert(0, dynamics.Dynamic(dyn_map[m]))

        chord_sym = chord_for(sec, bar, m)
        ...
        for pname, content_fn in [
            ...
            if m == 71:
                mx.insert(0, dynamics.Dynamic("ff"))
            for e in content_fn():
                mx.append(e)
...
def main():
    ...
    out = os.path.join(base, "..", "musicxml", "V3_The_Uncooperative_Groove_Final.musicxml")
