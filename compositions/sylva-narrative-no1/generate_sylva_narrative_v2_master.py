#!/usr/bin/env python3
"""
Sylva Narrative V2 – Master (GCE ≥ 9.7)
Orbit Album – Guitar + String Quartet

Transform V1 into fully developed narrative chamber work.
Four phases: I quiet emergence, II destabilisation, III dramatic peak, IV reflective transformation.
Central motif: E-F#-A-B (inversion, augmentation, expansion, fragmented echoes).
Living counterpoint. Guitar as colour instrument. ECM aesthetic.
"""
import os
from music21 import (
...
# Phase map: I(1-22) II(23-50) III(51-75) IV(76-108)
def phase(m):
    if m <= 22: return "I"
    if m <= 50: return "II"
    if m <= 75: return "III"
...
# === GUITAR V2: Colour instrument — dyads, harmonics, sparse ===
...
# === VIOLIN I: Primary narrative voice — motif E-F#-A-B (B5-F#5-A5-E5) ===
...
# === VIOLIN II: Motivic echoes, tension-building fragments ===
...
# === VIOLA: Contrary motion, inner harmonic colour ===
...
# === CELLO: Gravitational bass, pedal anchors ===
...
def build_score():
    ...
    dyn_map = {1: "pp", 12: "mp", 23: "mp", 35: "mf", 51: "mf", 60: "f", 70: "f", 76: "mf", 90: "p", 101: "pp"}
    colour_vn1 = 35
    colour_vla = 60
    colour_gtr = 95
    ...
    for m in range(1, 109):
        ...
        for pname, content_list in [
            ...
            if pname == "vn1" and m == colour_vn1:
                mx.insert(0, expressions.TextExpression("harm."))
            if pname == "vla" and m == colour_vla:
                mx.insert(0, expressions.TextExpression("sul tasto"))
...
def main():
    ...
    out_ecm = os.path.join(ws_root, "ECM-Orbit-Album-2027", "Compositions", "Sylva_Narrative", "V2_Sylva_Narrative_Master.musicxml")
    os.makedirs(os.path.dirname(out_ecm), exist_ok=True)

    out_local = os.path.join(base, "musicxml", "V2_Sylva_Narrative_Master.musicxml")
